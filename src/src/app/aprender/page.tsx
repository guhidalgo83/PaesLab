...
export default function LearnHomePage() {
  ...
  <article className="mt-6 rounded-3xl border border-indigo-300/20 bg-indigo-300/[0.05] p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-black text-indigo-200">Progreso Matemática M1</p><p className="mt-2 text-sm text-slate-400">{completed} de {total} lecciones completadas</p></div><a href="/ruta-estudio" className="rounded-xl bg-indigo-300 px-5 py-3 font-black text-slate-950">Ver ruta recomendada</a></div><div className="mt-5"><ProgressBar value={progress} /></div></article>
  <div className="mt-10 grid gap-5 md:grid-cols-2">{m1.map(unit=>{const pct=unit.lessonCount?Math.round(unit.completedCount/unit.lessonCount*100):0;return <a key={unit.id} href={`/aprender/m1/${unit.slug}`} className="group rounded-3xl border border-white/10 bg-white/[0.05] p-7 transition hover:-translate-y-1 hover:border-teal-300"><div className="flex items-start justify-between gap-4"><span className="grid h-14 w-14 place-items-center rounded-2xl bg-teal-300/10 text-xl font-black text-teal-300">{unit.icon}</span><span className="text-sm font-bold text-slate-500">{unit.lessonCount} lecciones</span></div><h2 className="mt-6 text-2xl font-black group-hover:text-teal-200">{unit.title}</h2><p className="mt-3 leading-7 text-slate-400">{unit.description}</p><div className="mt-6"><ProgressBar value={pct} label={`${unit.completedCount} completadas`} /></div></a>})}</div>
