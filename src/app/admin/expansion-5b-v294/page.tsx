...
import type {V294Overview} from "@/types/expansion-5b-v294";

export default function Expansion5BV294(){
  const router=useRouter(); const supabase=useMemo(()=>createClient(),[]);
  const [data,setData]=useState<V294Overview|null>(null); const [error,setError]=useState("");
  useEffect(()=>{let cancelled=false;void(async()=>{
    const {data:auth}=await supabase.auth.getUser(); if(cancelled)return;
    if(!auth.user){router.replace("/login");return;}
    const {data:payload,error:e}=await supabase.rpc("get_v294_5b_expansion_overview");
    if(cancelled)return; if(e)setError(e.message);else setData(payload as V294Overview);
  ...
  return <main className="min-h-screen bg-slate-950 text-white"><MathLabsHeader/>
    <section className="mx-auto max-w-7xl px-5 py-10"><V22AdminNav/>
      <header className="mt-6 rounded-[2.5rem] border border-cyan-300/20 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,.15),transparent_38%),#0f172a] p-7 sm:p-10">
        <p className="text-xs font-black uppercase tracking-[.22em] text-cyan-200">v29.0 → v29.4 · 5.º básico REVIEWED RC2 · Contract-audited staging</p>
        <h1 className="mt-3 text-4xl font-black sm:text-6xl">5B RC2 auditable sobre la arquitectura multi-curso</h1>
        <p className="mt-4 max-w-4xl leading-7 text-slate-300">27 OA → 54 topics distintos → soluciones y contratos exactos → 1404 ítems → adaptación → diagnóstico 54/54 → rutas. Representaciones y laboratorios permanecen como especificaciones; Q4 DB provisional 78. Snapshot curricular 2026-08-12 y revalidación obligatoria antes de release.</p>
      </header>
      {error?<p className="mt-5 rounded-2xl border border-rose-300/20 bg-rose-300/10 p-4">{error}</p>:null}
      {!data?<p className="mt-6 text-slate-400">Cargando macrobloque 5B…</p>:<>
