...
import type {V284Overview} from "@/types/expansion-7b-v284";

export default function Expansion7BV284(){
  const router=useRouter(); const supabase=useMemo(()=>createClient(),[]);
  const [data,setData]=useState<V284Overview|null>(null); const [error,setError]=useState("");
  useEffect(()=>{let cancelled=false;void(async()=>{
    const {data:auth}=await supabase.auth.getUser(); if(cancelled)return;
    if(!auth.user){router.replace("/login");return;}
    const {data:payload,error:e}=await supabase.rpc("get_v284_7b_expansion_overview");
    if(cancelled)return; if(e)setError(e.message);else setData(payload as V284Overview);
  ...
  return <main className="min-h-screen bg-slate-950 text-white"><MathLabsHeader/>
    <section className="mx-auto max-w-7xl px-5 py-10"><V22AdminNav/>
      <header className="mt-6 rounded-[2.5rem] border border-cyan-300/20 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,.15),transparent_38%),#0f172a] p-7 sm:p-10">
        <p className="text-xs font-black uppercase tracking-[.22em] text-cyan-200">v28.0 → v28.4 · 7.º básico REVIEWED RC2 · Contract-audited staging</p>
        <h1 className="mt-3 text-4xl font-black sm:text-6xl">7B RC2 auditable sobre la arquitectura multi-curso</h1>
        <p className="mt-4 max-w-4xl leading-7 text-slate-300">19 OA → 38 topics distintos → soluciones y contratos exactos → 988 ítems → adaptación → diagnóstico 38/38 → rutas. Representaciones y laboratorios permanecen como especificaciones; Q4 DB provisional 78. Snapshot curricular 2026-08-12 y revalidación obligatoria antes de release.</p>
      </header>
      {error?<p className="mt-5 rounded-2xl border border-rose-300/20 bg-rose-300/10 p-4">{error}</p>:null}
      {!data?<p className="mt-6 text-slate-400">Cargando macrobloque 7B…</p>:<>
