...
master=load("content_specs/curriculum_master_7b_v280.json")
coverage=load("content_specs/curriculum_coverage_7b_v280.json")
deep=load("content_specs/deep_content_7b_v280.json")
keys=load("content_specs/guided_answer_keys_7b_v280.json")
items=load("content_specs/assessment_items_7b_v281.json")
review_sets=load("content_specs/review_sets_7b_v281.json")
metadata=load("content_specs/adaptive_metadata_7b_v282.json")
diagnostic=load("content_specs/diagnostic_blueprint_7b_v283.json")
routes=load("content_specs/learning_routes_7b_v284.json")
cert=load("content_specs/assessment_certification_7b_v284.json")
snapshot=load("content_specs/curriculum_source_snapshot_7b_v284.json")
manifest=load("MANIFEST_V280_V284.json")

# Manifest and package bytes.
gate(manifest["release"]=="REVIEWED_RC2_COMPAT3","manifest release")
gate(len(manifest["files"])==21,"manifest must list 21 non-manifest artifacts")
listed={record["path"] for record in manifest["files"]}
actual={rel for rel in listed if (PACKAGE_ROOT/rel).is_file()}
...
gate(master["course_id"]=="cl-7-basico","course_id")
gate(len(master["oas"])==19,"OA count")
...
gate(len(snapshot["source_fingerprints"])==19,"source fingerprints")
...
gate(len(deep["topics"])==38,"topic count")
...
gate(len(master["nodes"])==152 and len(coverage)==152,"node/coverage count")
...
gate(len(keys)==304,"guided contracts")
...
for topic in deep["topics"]:
    ...
    for guided in topic["guided_practice"]:
        ...
        gate(key["validation_type"]==("mixed" if key["expected_numbers"] else "formula"),"guided validation type")
...
gate(len(items)==988,"assessment count")
by_kind=Counter(row["kind"] for row in items)
gate(by_kind=={"practice":760,"diagnostic":114,"review":114},"assessment mix")
...
gate(answers=={"A":247,"B":247,"C":247,"D":247},"answer balance")
...
gate({item["depth"] for item in items}=={1,2,3},"assessment DB depth contract")
gate({item["depth"] for item in metadata}=={1,2,3},"adaptive DB depth contract")
gate(all(m["calibration_status"]=="heuristic_candidate" and m["status"]=="staging" for m in metadata),"metadata staging")
gate(len(diagnostic["candidate_item_ids"])==114,"diagnostic candidates")
gate(diagnostic["min_items"]==19 and diagnostic["target_items"]==19 and diagnostic["default_limit"]==19,"diagnostic OA target/default")
...
gate(len(review_sets)==4 and len(routes)==38 and len(cert)==38,"routes/review/certification")
...
preflight=(PACKAGE_ROOT/"supabase/137a_preflight_mathlabs_v280_v284_7b_rc2.sql").read_text(encoding="utf-8")
installer=(PACKAGE_ROOT/"supabase/137_mathlabs_v280_v284_7b_macroblock_full_install.sql").read_text(encoding="utf-8")
rollback=(PACKAGE_ROOT/"supabase/137b_rollback_mathlabs_v280_v284_7b_rc2.sql").read_text(encoding="utf-8")
verify=(PACKAGE_ROOT/"supabase/138_verify_mathlabs_v280_v284_7b_macroblock.sql").read_text(encoding="utf-8")
...
page=(PACKAGE_ROOT/"src/app/admin/expansion-7b-v284/page.tsx").read_text(encoding="utf-8")
types=(PACKAGE_ROOT/"src/types/expansion-7b-v284.ts").read_text(encoding="utf-8")
gate("REVIEWED RC2" in page and "get_v284_7b_expansion_overview" in page,"admin page")
gate("export type V284Overview" in types,"types")

print("MathLabs v28.0–v28.4 · 7B REVIEWED RC2 COMPAT3 gate OK")
print("19 OA · 38 topics distintos · 152 nodos partial/pending human review")
print("988 ítems · 988 stems únicos · 988 contratos únicos · claves recalculadas")
print("A/B/C/D = 247/247/247/247 · 0 placeholders · 0 opciones equivalentes")
print("Worked exactos 228 · Guided exactos 304 · specs visuales 114 · specs labs 38")
print("Diagnóstico objetivo 19 OA · {:.2f} min / 45 min".format(diagnostic["estimated_target_minutes"]))
