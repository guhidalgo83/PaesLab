...
master=load("content_specs/curriculum_master_6b_v285.json")
coverage=load("content_specs/curriculum_coverage_6b_v285.json")
deep=load("content_specs/deep_content_6b_v285.json")
keys=load("content_specs/guided_answer_keys_6b_v285.json")
items=load("content_specs/assessment_items_6b_v286.json")
review_sets=load("content_specs/review_sets_6b_v286.json")
metadata=load("content_specs/adaptive_metadata_6b_v287.json")
diagnostic=load("content_specs/diagnostic_blueprint_6b_v288.json")
routes=load("content_specs/learning_routes_6b_v289.json")
cert=load("content_specs/assessment_certification_6b_v289.json")
snapshot=load("content_specs/curriculum_source_snapshot_6b_v289.json")
manifest=load("MANIFEST_V285_V289.json")
...
gate(master["course_id"]=="cl-6-basico","course_id")
gate(len(master["oas"])==24,"OA count")
...
gate(len(snapshot["source_fingerprints"])==24,"source fingerprints")
...
gate(len(deep["topics"])==48,"topic count")
...
gate(len(master["nodes"])==192 and len(coverage)==192,"node/coverage count")
...
gate(len(keys)==384,"guided contracts")
...
gate(len(items)==1248,"assessment count")
by_kind=Counter(row["kind"] for row in items)
gate(by_kind=={"practice":960,"diagnostic":144,"review":144},"assessment mix")
...
gate(answers=={"A":312,"B":312,"C":312,"D":312},"answer balance")
...
gate(len(diagnostic["candidate_item_ids"])==144,"diagnostic candidates")
gate(diagnostic["min_items"]==24 and diagnostic["target_items"]==24 and diagnostic["default_limit"]==24,"diagnostic OA target/default")
...
gate(len(review_sets)==4 and len(routes)==48 and len(cert)==48,"routes/review/certification")
...
preflight=(PACKAGE_ROOT/"supabase/139a_preflight_mathlabs_v285_v289_6b_rc2.sql").read_text(encoding="utf-8")
installer=(PACKAGE_ROOT/"supabase/139_mathlabs_v285_v289_6b_macroblock_full_install.sql").read_text(encoding="utf-8")
rollback=(PACKAGE_ROOT/"supabase/139b_rollback_mathlabs_v285_v289_6b_rc2.sql").read_text(encoding="utf-8")
verify=(PACKAGE_ROOT/"supabase/140_verify_mathlabs_v285_v289_6b_macroblock.sql").read_text(encoding="utf-8")
...
page=(PACKAGE_ROOT/"src/app/admin/expansion-6b-v289/page.tsx").read_text(encoding="utf-8")
types=(PACKAGE_ROOT/"src/types/expansion-6b-v289.ts").read_text(encoding="utf-8")
gate("REVIEWED RC2" in page and "get_v289_6b_expansion_overview" in page,"admin page")
gate("export type V289Overview" in types,"types")

print("MathLabs v28.5–v28.9 · 6B REVIEWED RC2 gate OK")
print("24 OA · 48 topics distintos · 192 nodos partial/pending human review")
print("1248 ítems · 1248 stems únicos · 1248 contratos únicos · claves recalculadas")
print("A/B/C/D = 312/312/312/312 · 0 placeholders · 0 opciones equivalentes")
print("Worked exactos 288 · Guided exactos 384 · specs visuales 144 · specs labs 48")
print("Diagnóstico objetivo 24 OA · {:.2f} min / 45 min".format(diagnostic["estimated_target_minutes"]))
