-- MathLabs v29.0 → v29.4 · 5B RC2 · TARGETED ROLLBACK
...
do $$
begin
 if (select count(*) from public.deep_learning_paths_v250 where course_id='cl-5-basico' and content_version='v29.0' and status='staging')<>54
    or (select count(*) from public.multigrade_assessment_items_v261 where course_id='cl-5-basico' and status='staging' and not is_published)<>1404
    or exists(select 1 from public.deep_learning_paths_v250 where course_id='cl-5-basico' and status='published')
   ...
   raise exception 'Rollback 5B RC2 rechazado: estado distinto del staging exacto esperado';
 end if;
...
select topic_id from public.deep_learning_paths_v250 where course_id='cl-5-basico' and content_version='v29.0';
create temporary table rc2_objectives on commit drop as
select id from public.curriculum_objectives where course_id='cl-5-basico';

delete from public.multigrade_assessment_readiness_v264 where course_id='cl-5-basico';
...
delete from public.multigrade_assessment_certification_v264 where course_id='cl-5-basico';
delete from public.multigrade_learning_route_contract_v264 where course_id='cl-5-basico';
delete from public.multigrade_diagnostic_blueprints_v263 where course_id='cl-5-basico';
delete from public.multigrade_adaptive_item_metadata_v262 where course_id='cl-5-basico';
delete from public.multigrade_review_sets_v261 where course_id='cl-5-basico';
delete from public.multigrade_assessment_items_v261 where course_id='cl-5-basico';
delete from public.deep_guided_answer_keys_v253 where topic_id in (select topic_id from rc2_topics);
delete from public.curriculum_topic_readiness_v254 where course_id='cl-5-basico';
delete from public.curriculum_coverage_v254 where course_id='cl-5-basico';
delete from public.curriculum_nodes_v254 where course_id='cl-5-basico';
delete from public.deep_content_quality_v253 where topic_id in (select topic_id from rc2_topics);
delete from public.deep_learning_paths_v250 where course_id='cl-5-basico';
delete from public.curriculum_objective_topics where objective_id in (select id from rc2_objectives);
delete from public.knowledge_topics where course_id='cl-5-basico';
delete from public.curriculum_objectives where course_id='cl-5-basico';
delete from public.curriculum_axes where course_id='cl-5-basico';
delete from public.curriculum_oas_v254 where course_id='cl-5-basico';
delete from public.curriculum_sources_v254 where course_id='cl-5-basico';
delete from public.multigrade_curriculum_source_snapshot_v269 where course_id='cl-5-basico';
delete from public.multigrade_course_runtime_gate_v269 where course_id='cl-5-basico';

update public.mathlabs_course_catalog_v259 set lifecycle_status='planned',content_version=null,updated_at=now() where course_id='cl-5-basico';
update public.mathlabs_course_import_contract_v259 set curriculum_import_status='pending',expected_oa_count=null,expected_topic_count=null,
 source_reference=null,next_action='Regenerar o reinstalar 5B RC2 después de nueva revisión.',updated_at=now() where course_id='cl-5-basico';
update public.mathlabs_course_module_readiness_v259 set readiness_status='planned',refreshed_at=now() where course_id='cl-5-basico';

drop function if exists public.get_v294_5b_expansion_overview();
drop function if exists public.recompute_deep_content_quality_v290_5b(text);
...
or exists(select 1 from public.multigrade_course_runtime_gate_v269 where course_id='cl-5-basico') then
   raise exception 'Rollback 5B RC2 incompleto; transaction will roll back';
 end if;
 if (select count(*) from public.deep_learning_paths_v250 where course_id='cl-8-basico' and content_version='v27.5' and status='staging')<>34
   or (select count(*) from public.curriculum_nodes_v254 where course_id='cl-8-basico' and is_active)<>136
   or (select count(*) from public.multigrade_assessment_items_v261 where course_id='cl-8-basico')<>884
