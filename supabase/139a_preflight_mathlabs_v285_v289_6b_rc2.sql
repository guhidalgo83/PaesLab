-- MathLabs v28.5 → v28.9 · 6B RC2 · READ-ONLY PREFLIGHT
-- This file performs no INSERT, UPDATE, DELETE, CREATE, ALTER, DROP or TRUNCATE.
select public.mathlabs_assert_admin_v22();

do $$
begin
 if (select count(*) from public.courses where id='cl-6-basico')<>1 then
   raise exception 'Preflight RC2: cl-6-basico no existe en public.courses';
 end if;
 ...
 if (select count(*) from public.mathlabs_course_catalog_v259 where course_id='cl-6-basico')<>1 then
   raise exception 'Preflight RC2: cl-6-basico no está registrado en el catálogo';
 end if;
...
or exists(select 1 from public.multigrade_course_runtime_gate_v269 where course_id='cl-6-basico') then
   raise exception 'Preflight RC2: cl-6-basico contiene artefactos previos; no instalar sobre estado parcial';
 end if;
